...
# === تعديل جديد: دالة تشخيص تلتقط سجل yt-dlp الداخلي الكامل (verbose) ===
def debug_youtube_probe(video_url: str) -> Dict[str, Any]:
    """
    يشغّل yt-dlp بوضع verbose على فيديو يوتيوب حقيقي (بدون تحميل)،
    ويلتقط كل رسائله الداخلية، عشان نتأكد هل POT provider اتسجّل فعليًا
    عند yt-dlp أو لا. تشخيصي فقط، ما يغيّر أي سلوك تشغيلي حقيقي.
    """
    captured_messages: List[str] = []

    class _CapturingLogger:
        def debug(self, msg):
            captured_messages.append(f"[debug] {msg}")

        def info(self, msg):
            captured_messages.append(f"[info] {msg}")

        def warning(self, msg):
            captured_messages.append(f"[warning] {msg}")

        def error(self, msg):
            captured_messages.append(f"[error] {msg}")
    ...
    options["logger"] = _CapturingLogger()

    result: Dict[str, Any] = {"video_url": video_url}
    try:
        with yt_dlp.YoutubeDL(options) as ydl:
    ...
    result["log_lines"] = captured_messages
    result["pot_related_lines"] = [
        line
...
# === تعديل جديد: دالة تستخرج معلومات تشخيصية عن بيئة حل تحدي التوقيع (JS runtime) ===
def debug_youtube_js_runtime_status() -> Dict[str, Any]:
    """
    فحص سريع (بدون أي اتصال بيوتيوب) لحالة أدوات حل تحدي التوقيع:
    هل deno موجود فعليًا بالسيرفر، ومساره. تشخيصي فقط.
    """
    deno_path = shutil.which("deno")
    return {
        "deno_found": bool(deno_path),
        "deno_path": deno_path,
        "note": (
            "لو deno_found=false، معناها JS runtime غير متوفر أصلًا بالسيرفر، "
            "وهذا يفسر فشل Signature/n challenge solving بشكل دائم أو متقطع "
            "(متقطع لو المكوّن ejs:github نفسه يعتمد على تحميل شبكي وقت "
            "التشغيل ويفشل أحيانًا)."
        ),
    }
...
def debug_youtube_direct_link(video_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # نرجّع الخطأ بالرد نفسه بدل ما يطلع 500 عام بدون تفاصيل
        return {
            "success": False,
